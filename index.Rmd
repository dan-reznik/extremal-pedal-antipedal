...
Steiner proves the area will be maximal (resp. minimal) if $\sum_i{sin(2\theta_i)}$ is positive (resp.  negative). Notice that for parallelograms, the sum of double-angle sines is zero. Indeed, the area of pedal polygons to this shape is invariant over $P$. 
...
The minimal-area antipedal must occur with respect to the Orthocenter $X_4$ of the reference triangle. To see this, recall a theorem by Gallatly, page 58 [@gallatly1914]:
...
To see this, recall $X_4$ is the isogonal conjugate of $X_3$. Suppose another point $Y$ existed with respect to which an antipedal had area lower than $A^*$. With $A$ constant, this would imply a pedal with respect to $Y^*$, the isogonal of $Y$, of area higher than $A'$, a contradiction since $A'$ is maximal.

To summarize: given a triangle, the antipedal with respect to $X_4$ is its the anticomplementary triangle [@mw], so the minimal area is $4A$. As before, the antipedal's maximal-area pedal polygon will be its medial, i.e., the original $T$ itself.
...
The figure below shows the non-linear isocurves of area for the antipedal triangle with respect to points in the plane are shown below for three triangles (acute, right, and obtuse). Note the zero-area iso-curve along the circumcircle, where the antipedal triangle shrinks to a point.
